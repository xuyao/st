...
//	String[] args={"zzf","1yzf","3yzf","6yzf","1nzf","2nzf","jnzf","3nzf"};
...
//		List<String> list1 = year();
	...
	public List<String> guding(){
		...
		for(String arg : args){
			...
			String url = "http://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft=all&rs=&gs=0&sc="+arg+"&st=desc&sd="+date+
					"&ed="+date+"&qdii=&tabSubtype=,,,,,&pi=1&pn=2500&dx=1&v=0."+NumberUtil.getRandomNum(17);
			try {
	...
	public List<String> year(){
		...
		for(String[] ys : year){
			...
			String url = "http://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft=all&rs=&gs=0&sc=qjzf&st=desc&sd="+ys[0]+
					"&ed="+ys[1]+"&qdii=&tabSubtype=,,,,,&pi=1&pn=2500&dx=1&v=0."+NumberUtil.getRandomNum(17);
			try {
				ArrayList<String> listfund = new ArrayList<String>();
				String result = http.sendGet(url, nameValuePairList);
				result = StrUtils.subString(result, "[\"", "\"]");
